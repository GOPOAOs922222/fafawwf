...
if ($_SESSION["login"]){
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $categorie = $_POST["name-categorie"];

        $query_1 = "SELECT * FROM categories WHERE categorie_name = '$categorie'";
        $stmt_1 = $pdo->query($query_1);
        $user = $stmt_1->fetch(PDO::FETCH_ASSOC);
        if (strlen($user["id"]) >= 1){
            $_SESSION["exist"] = 'This Categorie is already exist';
            header("Location: upload_categorie.php");
            return;
        }elseif (strlen($user["id"]) == 0){
            $query = "INSERT INTO categories(categorie_name, banner) VALUES (:n, :p);";
            $stmt = $pdo->prepare($query);
            $stmt->execute(array(':n' => $categorie, ':p' => $image,));

            $query_2 = "SELECT * FROM categories WHERE categorie_name = '$categorie'";
            ...
            if (strlen($user_2["id"]) == 0){
                ...
                $_SESSION["success_upload"] = 'The categorie has been uploaded successfully';
                header("Location: upload_categorie.php");
                return;
            }
        }
...
<!--                    name categorie, upload banner-->
                    <form method="post" enctype="multipart/form-data">
                        <h3 class="my-5 text-center">Upload New Categorie</h3>
                        ...
                        <div class="row mt-4">
                            <div class="col-md-6 offset-md-3 col-10 offset-1 text-center px-0 py-0">
                                ...
                                <?php }else if(isset($_SESSION["success_upload"])){?>
                                    <div class="alert alert-success">
                                        <h6><?php echo $_SESSION["success_upload"] ?></h6>
                                    </div>
                                    <?php unset($_SESSION["success_upload"])?>
                                <?php }?>
                            </div>
                        </div>
...
<!--                        name-->
                        <div class="row mb-3">
                            <div class="col-md-3">
                                <h6 class="mt-2">Categorie Name</h6>
                            </div>
                            <div class="col-md-9">
                                <input type="text" name="name-categorie" class="form-control" placeholder="Categorie Name..." required>
                            </div>
                        </div>
<!--                        upload images-->
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <h6 class="mt-2">Banner Categorie (<span class="text-danger">One Image</span>)</h6>

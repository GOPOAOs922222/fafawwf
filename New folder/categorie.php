...
if (isset($_GET["categorie"])) {
    ...
    $query_banner = "SELECT * FROM categories WHERE categorie_name LIKE '%$categorie%'";
    $stmt_banner = $pdo->query($query_banner);
    $show_banner = $stmt_banner->fetch(PDO::FETCH_ASSOC);
    if (strlen($show_banner["id"]) == 0) {
        header("Location: categorie.php?categorie=مواقع وصحف واخبار&show=30&page=1");
        return;
    }
}

if (isset($_GET["categorie"]) AND isset($_GET["show"])) {
    $show = $_GET["show"];
    $categorie = $_GET["categorie"];
    if ($show == "all") {
        $show = 9999999999;
    ...
    $query_comment = "
            ...
            ";
    $stmt_comment = $pdo->query($query_comment);
    $cards_comment = $stmt_comment->fetchall(PDO::FETCH_ASSOC);


    $query_cards_number = "SELECT COUNT(*) AS count_cards FROM websites WHERE categorie LIKE '$categorie'";
    $stmt_cards_number = $pdo->query($query_cards_number);
    $total_cards = $stmt_cards_number->fetch(PDO::FETCH_ASSOC);

    $query_count_cards = "SELECT * FROM websites WHERE categorie LIKE '$categorie' LIMIT $show";
...
//    echo $end;
    ...
    $query_cards = "SELECT DISTINCT * FROM websites WHERE categorie LIKE '$categorie' LIMIT $end offset $start";
    ...
    if (!isset($_GET["show"])) {
        ...
        $show = $_GET["show"];
        header("Location: categorie.php?categorie=مواقع وصحف واخبار&show=$show&page=1");
        return;
    }
}
?>
...
<head>
    ...
    <title><?php echo $show_banner["categorie_name"];?></title>
        ...
        <div class="text-black opacity-75 mt-2 mx-3">
            <h6>
                ...
                <span class="text-dark opacity-75">
                    &nbsp; / &nbsp;<a href="index.php" class="text-decoration-none text-dark opacity-75"> Home </a> &nbsp; / &nbsp;<?php echo $show_banner["categorie_name"];?>
                </span>
            </h6>
        </div>
        <br>
...
<!--                                categories-->
                                <div class="mb-4">
                                    <?php foreach ($categories as $cate){?>
                                        <h6 class="mb-3"><a href="categorie.php?categorie=<?php echo $cate["categorie_name"]?>&show=30&page=1" class="text-decoration-none categorie-text">->  <?php echo $cate["categorie_name"]?></a></h6></a>
                                    <?php }?>
                                </div>
<!--                                best comments-->
                                <div>
                                    <h5>Best Comments</h5>
                                    <hr>
                                    <?php foreach ($cards_comment as $comment){?>
                                        <div class="card mb-3">
                                            <div class="card-header">
                                                <div class="row">
                                                    <div class="col-7 fw-bolder">
                                                ...
                                                <img src="stars/<?php echo $comment["rate"]?>star.png" width="100" height="15" class="text-center">
                                            </div>
                                            <div class="card-body">
                                                <h6><?php echo $comment["review"]?></h6>
                                            </div>
                                        </div>
                                    <?php }?>
                                </div>
                ...
                <div class="col-md-9">
                    <!--banner categorie and name categorie-->
                    <div>
                        <h5 class="text-dark fw-bolder opacity-75 mb-4 mt-md-0 mt-3"><?php echo $show_banner["categorie_name"];?></h5>

                        <img src="data:image/<?php echo "png"; ?>;base64,<?php echo base64_encode($show_banner["banner"]); ?>" id="banner">
                    </div>
                    <!--filter show-->
                    <div class="alert alert-warning py-0" style="background: #fff1e5">
                        <form method="get" class="mb-3">
                            <div class="row mt-4">
                                <div class="col-md-1 offset-md-8 col-2 offset-3">
                                ...
                                <div class="col-md-1 col-3">
                                    <button type="submit" class="btn btn-warning text-white fw-bolder">Submit</button>
                                </div>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="row mt-4 mb-3">
                        <?php if (count($cards) == 0){?>
                            <h5 class="text-center">There is no result in this page</h5>
                        <?php }else{?>
                            <?php foreach ($cards as $card){?>
                                <a href="website.php?categorie=<?php echo $card["categorie"]?>&website=<?php echo $card["name_site"]?>" class="col-md-2 mb-3 py-md-3 col-4" id="card-categorie">
                                    <img src="data:image/<?php echo "png"; ?>;base64,<?php echo base64_encode($card["logo"]); ?>" class="border border-dark rounded-3 card-categorie">
                                </a>
                            <?php }?>
                        <?php }?>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-11 offset-md-1 text-center">
                                <?php for ($o=1 ; $o<=$count_pages ; $o++){?>
                                    ...
                                    <?php }else{?>
                                        <a href="<?php echo "categorie.php?categorie=$categorie&show=$show&page=$o"?>" class="text-decoration-none"><button class="btn btn-outline-danger mb-2 page-btn"><?php echo $o?></button></a>
                                    <?php }?>
                                <?php }?>
                            </div>
                        </div>

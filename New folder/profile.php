...
if (isset($_SESSION["login"])){
    ...
    $query_d = "SELECT * FROM user_v1 WHERE email = '$e_mail_log' AND password = '$password_log'";
    $stmt_d = $pdo->query($query_d);
    $user_d = $stmt_d->fetch(PDO::FETCH_ASSOC);

    if ($_SERVER["REQUEST_METHOD"] == "POST" AND isset($_POST["submit"])) {
        $f_name = $_POST["f-name"];
        $l_name = $_POST["l-name"];
        $o_password = $_POST["password"];
        $n_password = $_POST["c-password"];


        if (strlen($n_password) > 6 AND strlen($f_name) < 15 AND strlen($l_name) < 15) {
            if ($o_password == $password_log){
                $done = true;
            }else {
                $errors_reg = 'The old Password is incorrect';
            }
        }else {
            $errors_reg = 'Something is wrong please try again';
        }
        ...
        if (count($errors_reg) > 0) {
            ...
            $query = "UPDATE user_v1 SET password='$n_password', first_name='$f_name', last_name='$l_name' WHERE  email=:e AND password=:p;";
            $stmt = $pdo->prepare($query);
            $stmt->execute(array(':e' => $e_mail_log, ':p' => $password_log,));


            $query_c = "SELECT * FROM user_v1 WHERE email = '$e_mail_log' AND password = '$n_password'";
            $stmt_c = $pdo->query($query_c);
            $user_one = $stmt_c->fetch(PDO::FETCH_ASSOC);
            if (strlen($user_one["id"]) == 0){
                $_SESSION["error"] = 'Something is wrong please try again';
                header("Location: profile.php");
                return;
            }else if (strlen($user_one["id"]) >= 1){
                $_SESSION["edit_success"] = 'Profile Edit Success';
                $_SESSION["password_login"] = $n_password;
                header("Location: profile.php");
                return;
            }
        }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" AND isset($_POST["save"])) {
        ...
        $link_site = $_POST["link-site"];
        $mci = $_POST["mci"];
        $proof = $_POST["proof"];


        if (strlen($fullname) > 9){
            if (filter_var($link_site, FILTER_VALIDATE_URL)) {
                if (strlen($mci) > 6) {
                    if (strlen($proof) > 9) {
                        $done = true;
                    } else {
                        $errors_trad = 'Please enter valid proof number';
                    }
                } else {
                    $errors_trad = 'The commercial record number is incorrect';
                }
            } else {
                $errors_trad = 'Please enter valid website';
            }
        } else {
            $errors_trad = 'Please enter real fullname';
        }

        if (count($errors_trad) > 0) {
            $_SESSION["error_trad"] = $errors_trad;
            header("Location: profile.php");
            return;
        } else if (!isset($_SESSION["error"]) && isset($done)) {
            $query_c = "SELECT * FROM user_v1 WHERE email = '$e_mail_log' AND password = '$password_log' AND user_status = '1'";
            $stmt_c = $pdo->query($query_c);
            $user_one = $stmt_c->fetch(PDO::FETCH_ASSOC);

            if (strlen($user_one["id"]) == 0){
                $query = "INSERT INTO user_v1(mci, website, proof_number, fullname) VALUES (:n, :p, :c, :j);";
                $stmt = $pdo->prepare($query);
                $stmt->execute(array(':n' => $mci, ':p' => $link_site, ':c' => $proof, ':j' => $fullname,));
                $_SESSION["edit_success_trad"] = 'Trader details added successfully';
                header("Location: profile.php");
                return;
            }else if (strlen($user_one["id"]) >= 1){
                $query = "UPDATE user_v1 SET mci='$mci', website='$link_site', proof_number='$proof' , fullname='$fullname' WHERE  email=:e AND password=:p;";
                $stmt = $pdo->prepare($query);
                $stmt->execute(array(':e' => $e_mail_log, ':p' => $password_log,));
                $_SESSION["edit_success_trad"] = 'Information updated successfully';
                header("Location: profile.php");
                return;
            }
        }
        ...
        <div class="text-black opacity-75 mt-2 mx-3">
            <h6>
                ...
                <span class="text-dark opacity-75">
                    &nbsp; / &nbsp;<a href="index.php" class="text-decoration-none text-dark opacity-75"> Home </a> &nbsp; / &nbsp;Profile
                </span>
            </h6>
        </div>
            ...
            <div class="col-md-8">
                    ...
                    <div class="col-md-6 offset-md-3 col-10 offset-1 text-center px-0 py-0">
                        ...
                        <?php }elseif (isset($_SESSION["edit_success"])){?>
                            <div class="alert alert-success">
                                <h6><?php echo $_SESSION["edit_success"];?></h6>
                            </div>
                ...
                <form method="post">
                    <div class="row mb-md-3">
                        ...
                        <div class="col-md-4">
                            <input type="text" name="f-name" placeholder="First Name..." value="<?php echo $user_d["first_name"]?>" class="form-control">
                        </div>
                        <div class="col-md-2 mt-md-0 mt-2">
                            <h6 class="mt-2"><span class="text-danger"> * </span>Last Name</h6>
                        </div>
                        <div class="col-md-4">
                            <input type="text" name="l-name" placeholder="Last Name..." value="<?php echo $user_d["last_name"]?>" class="form-control">
                        </div>
                    </div>
                    <div class="row mb-md-3">
                        <div class="col-md-2 mt-md-0 mt-2">
                        ...
                        <div class="col-md-4">
                            <input type="text" value="<?php echo str_replace("-", "/", $user_d["birthday"])?>" name="date" class="form-control" disabled>
                        </div>
                        <div class="col-md-2 mt-md-0 mt-2">
                            <h6 class="mt-2"><span class="text-danger"> * </span>Email Address</h6>
                        </div>
                        <div class="col-md-4">
                            <input type="text" name="email" value="<?php echo $user_d["email"]?>" class="form-control" disabled>
                        </div>
                    </div>
                    <div class="row mb-md-3">
                        <div class="col-md-2 mt-md-0 mt-2">
                    ...
                    <div class="text-end mt-md-0 mt-3">
                        <button type="submit" class="btn btn-danger fw-bolder" name="submit" value="1">Submit</button>
                    </div>
                </form>
<!--                user membership-->
                ...
                <div class="row mb-5">
                    ...
                    <div class="col-md-4 col-auto">
                        <?php if ($user_d["user_status"] == 0){?>
                            <h6>Customer</h6>
                        <?php }elseif ($user_d["user_status"] == 1){?>
                            <h6>Trader</h6>
                        <?php }elseif ($user_d["user_status"] == 2){?>
                            <h6>VIP Trader</h6>
                        <?php }?>
                    </div>
                </div>
                <?php if ($user_d["user_status"] == 1 OR $user_d["user_status"] == 2){?>
                    <?php if ($user_d["user_status"] == 2){
                        $run = "disabled";
                    }else{
                        $run = "";
                    }?>
                    ...
                    <div class="row mt-4">
                        <div class="col-md-6 offset-md-3 col-10 offset-1 text-center px-0 py-0">
                            <?php if (isset($_SESSION["error_trad"])){?>
                                <div class="alert alert-danger">
                                    <h6><?php echo $_SESSION["error_trad"];?></h6>
                                </div>
                                <?php unset($_SESSION["error_trad"])?>
                            <?php }elseif (isset($_SESSION["edit_success_trad"])){?>
                                <div class="alert alert-success">
                                    <h6><?php echo $_SESSION["edit_success_trad"];?></h6>
                                </div>
                                <?php unset($_SESSION["edit_success_trad"])?>
                            <?php }?>
                        </div>
                    </div>
                    ...
                    <form method="post">
                        <div class="row mb-3">
                            <div class="col-md-3">
                                <h6 class="mt-2"><span class="text-danger"> * </span>FullName</h6>
                            </div>
                            <div class="col-md-5">
                                <?php if ($user_d["mci"] == null){?>
                                    <input type="text" placeholder="FullName..." name="fullname" class="form-control" <?php echo $run?>>
                                <?php }else{?>
                                    <input type="text" placeholder="FullName..." name="fullname" value="<?php echo $user_d["fullname"]?>" class="form-control" style="color: #676767" <?php echo $run?>>
                                <?php }?>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-3">
                                <h6 class="mt-2"><span class="text-danger"> * </span>Link Website</h6>
                            </div>
                            <div class="col-md-5">
                                <?php if ($user_d["mci"] == null){?>
                                    <input type="text" name="link-site" placeholder="https://examble.com" class="form-control" <?php echo $run?>>
                                <?php }else{?>
                                    <input type="text" name="link-site" placeholder="https://examble.com" value="<?php echo $user_d["website"]?>" class="form-control" style="color: #676767" <?php echo $run?>>
                                <?php }?>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-3">
                                <h6 class="mt-2"><span class="text-danger"> * </span>Commercial record (MCI)</h6>
                            </div>
                            <div class="col-md-5">
                                <?php if ($user_d["mci"] == null){?>
                                    <input type="text" placeholder="9671052*******" name="mci" class="form-control" <?php echo $run?>>
                                <?php }else{?>
                                    <input type="text" placeholder="9671052*******" name="mci" value="<?php echo $user_d["mci"]?>" class="form-control" style="color: #676767" <?php echo $run?>>
                                <?php }?>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-3">
                                <h6 class="mt-2"><span class="text-danger"> * </span>proof number</h6>
                            </div>
                            <div class="col-md-5">
                                <?php if ($user_d["mci"] == null){?>
                                    <input type="text" name="proof" placeholder="3010200500********" class="form-control" style="color: #676767" <?php echo $run?>>
                                <?php }else{?>
                                    <input type="text" name="proof" placeholder="3010200500********" value="<?php echo $user_d["proof_number"]?>" class="form-control" style="color: #676767" <?php echo $run?>>
                                <?php }?>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-1">
                                <button type="submit" name="save" value="1" class="btn btn-danger mt-3 fw-bolder">Save</button>
                            </div>
                            <div class="col-md-11 mt-4">
                                <input type="checkbox" name="privacy" value="1" class="mx-1 my-auto" required>
